...
export default function Contact() {
  ...
  const onSubmit = (e) => {
    ...
    alert(
      `Thank you ${formData.name}, with email ${formData.email} we will respond to the following message shortly: ${formData.message}`
    )
  }
  function nameValid(testText) {
    const re = /.{2,30}/
    const result = testText.match(re)
    if (result) {
      console.log("name is valid")
      return true
    } else {
      document.querySelector("label:has(#name)").classList.add("error")
      removeErrors()

      return false
    }
  }
  function emailValid(testText) {
    const re =
    ...
    if (result) {
      ...
      document.querySelector("label:has(#email)").classList.add("error")
      removeErrors()
      return false
    }
  }
  function messageValid(testText) {
    const re = /(.|\n){2,600}/
    const result = testText.match(re)
    if (result) {
      ...
      document.querySelector("label:has(#message)").classList.add("error")
      removeErrors()
      return false
    }
  }
  function removeErrors() {
    setTimeout(() => {
      document.querySelector("label.error").classList.remove("error")
    }, 5000)
  }
